package com.fuzzy.iotdb.feedback;
...
@Slf4j
public class IotDBQuerySynthesisFeedbackManager {
    ...
    static {
        // constant comparison column
        reportedBugSequence.add(") < (column)");
        reportedBugSequence.add(") > (column)");
        reportedBugSequence.add(") <= (column)");
        reportedBugSequence.add(") >= (column)");
        reportedBugSequence.add(") < (time)");
        reportedBugSequence.add(") > (time)");
        reportedBugSequence.add(") <= (time)");
        reportedBugSequence.add(") >= (time)");
    }

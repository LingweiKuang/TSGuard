...
@Slf4j
public class VMApiEntry implements ApiEntry {
    private String host;
    private int port;
    private int pushGatewayPort = 9091;
    private RestTemplate template = null;

    public VMApiEntry(String host, int port) {
        this.host = host;
        this.port = port;
        setUp();
    }
    ...
    public String executeGetRequest(URI url) {
        try {
            log.info("VM 执行查询, url:{}", URLDecoder.decode(url.toString(), StandardCharsets.UTF_8.toString()));
        } catch (Exception e) {
